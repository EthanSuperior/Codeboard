...
const basicDrawCalls = {
	...
	onchargedraw: function (percent) {
		...
		drawCircle(0, 0, 0, this.duration + this.size * percent, {
			fill: this.color,
		});
	...
	oncooldowndraw: function (percent) {
		ctx.globalCompositeOperation = "hue";
		drawCircle(0, 0, 0, this.size * (1 - percent), {
			stroke: this.color,
	...
	onactivedraw: function (percent) {
		ctx.globalCompositeOperation = "hue";
		drawCircle(0, 0, 0, this.size + this.deltaTimer / 2, {
			stroke: this.color,
			strokeWidth: this.deltaTimer,
		});
		ctx.globalCompositeOperation = "source-over";
	},
	ontickdraw: function (percent) {
		ctx.globalCompositeOperation = "hue";
		drawProgressCircle(0, 0, this.size, 0, percent, -Math.PI / 2, {
			fill: this.color,
		});
		ctx.globalCompositeOperation = "source-over";
	},
...
registerAbility("Bloodthirst_1", {
	...
	onactivate: function (player) {
		function selfheal(amount, target) {
			player.recover(
				amount * 2.5 * (player.health.missingPercent / 10),
				player
			);
		}
...
const classAbilities = {
	Fighter: {
		Both: {
			1: ["Heavy Blow", "Shoulder Check", "Dash"],
			2: ["Unshaken"],
			3: ["Juggernaut"],
		},
		Bezerker: {
